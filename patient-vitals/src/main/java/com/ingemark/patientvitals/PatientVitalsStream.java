package com.ingemark.patientvitals;

import com.ingemark.patientvitals.config.AppConfig;
import com.ingemark.patientvitals.joiner.PatientVitalsJoiner;
...
@EnableKafka
@EnableKafkaStreams
public class PatientVitalsStream {
    private static final Logger logger = LoggerFactory.getLogger(PatientVitalsStream.class);

    private final AppConfig appConfig;

    public PatientVitalsStream(AppConfig appConfig) {
    ...
    @Bean
    public KStream<String, PatientVitals> kStream(StreamsBuilder streamsBuilder,
                                                  Serde<String> stringSerde,
                                                  JsonSerde<HeartRate> heartRateJsonSerde,
                                                  JsonSerde<BodyTemp> bodyTempJsonSerde,
                                                  JsonSerde<PatientVitals> patientVitalsJsonSerde) {
        ...
        var patientVitalsJoiner = new PatientVitalsJoiner();
        ...
        patientVitalsJoinedStream.peek((key, value) -> logger.info("key[{}] value[{}]", key, value))
                .to(appConfig.getPatientVitalsSinkTopic(), Produced.with(stringSerde, patientVitalsJsonSerde));
    ...
    @Bean
    public JsonSerde<PatientVitals> patientVitalsJsonSerde() {
        return new JsonSerde<>(PatientVitals.class);
    }
}
